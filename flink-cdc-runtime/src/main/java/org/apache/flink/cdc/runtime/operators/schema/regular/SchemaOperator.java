...
public class SchemaOperator extends AbstractStreamOperator<Event>
    ...
    private void handleSchemaChangeEvent(SchemaChangeEvent originalEvent) throws Exception {
        ...
        LOG.info(
                "Processing schema change event for table {}: {}",
                tableId,
                originalEvent.getClass().getSimpleName());
    ...
    private void handleDataChangeEvent(DataChangeEvent dataChangeEvent) {
        ...
        for (TableId sinkTableId : routedTables) {
            ...
            LOG.info(
                    "Evolved schema for {}: {}",
                    sinkTableId,
                    evolvedSchema != null ? "FOUND" : "NULL");

            // ROBUST SCHEMA INITIALIZATION: Handle missing evolved schema for sink tables
            if (evolvedSchema == null) {
                if (originalSchema != null) {
                    LOG.info(
                            "Initializing evolved schema for sink table {} using original schema",
                            sinkTableId);
                    ...
                    LOG.warn(
                            "No original schema available for {}. Searching for compatible schema...",
                            tableId);
            ...
            if (evolvedSchema == null) {
                ...
                try {
                    ...
                    if (response != null && response.getEvolvedSchemas().containsKey(sinkTableId)) {
                        ...
                        evolvedSchemaMap.put(sinkTableId, evolvedSchema);
                        LOG.info(
                                "Successfully retrieved schema from coordinator for {}",
                                sinkTableId);
                    }
                } catch (Exception e) {
                ...
                if (evolvedSchema == null) {
                    LOG.warn(
                            "SKIPPING RECORD: Unable to resolve schema for table {}. Record will be dropped.",
                            sinkTableId);
    ...
    private <REQUEST extends CoordinationRequest, RESPONSE extends CoordinationResponse>
        ...
        for (int attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                LOG.debug(
                        "Sending request to coordinator (attempt {}/{}): {}",
                        attempt,
                        maxRetries,
                        request);
                ...
                if (attempt > 1) {
                    LOG.info(
                            "Successfully sent request to coordinator on attempt {}: {}",
                            attempt,
                            request);
                }
                ...
                if (attempt < maxRetries) {
                    LOG.warn(
                            "Coordinator request timed out on attempt {} (timeout: {}ms), retrying: {}",
                            attempt,
                            baseTimeoutMs * attempt,
                            request);
                    ...
                    try {
                        ...
                        throw new IllegalStateException(
                                "Interrupted while retrying coordinator request: " + request, ie);
                    }
                } else {
                    LOG.error(
                            "Coordinator request failed after {} attempts due to timeout: {}",
                            maxRetries,
                            request);
                    throw new IllegalStateException(
                            String.format(
                                    "Failed to send request to coordinator after %d attempts (final timeout: %dms): %s",
                                    maxRetries, baseTimeoutMs * maxRetries, request),
                            e);
                }
            } catch (Exception e) {
                if (attempt < maxRetries && isRetryableException(e)) {
                    LOG.warn(
                            "Coordinator request failed on attempt {} with retryable error, retrying: {} - Error: {}",
                            attempt,
                            request,
                            e.getMessage());
                    ...
                    LOG.error(
                            "Coordinator request failed on attempt {} with non-retryable error: {} - Error: {}",
                            attempt,
                            request,
                            e.getMessage(),
                            e);
        ...
        throw new IllegalStateException(
                "Unexpected end of retry loop for coordinator request: " + request);
    }
