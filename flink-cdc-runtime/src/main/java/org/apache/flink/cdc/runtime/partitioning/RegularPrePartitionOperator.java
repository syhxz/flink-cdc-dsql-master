...
public class RegularPrePartitionOperator extends AbstractStreamOperator<PartitioningEvent>
    ...
    private void partitionBy(DataChangeEvent dataChangeEvent) throws Exception {
        ...
        LOG.debug("Partitioning data change event for table: {}", tableId);
        
        try {
            HashFunction<DataChangeEvent> hashFunction = cachedHashFunctions.get(tableId);
            int partition = hashFunction.hashcode(dataChangeEvent) % downstreamParallelism;
            
            output.collect(
                    new StreamRecord<>(
                            PartitioningEvent.ofRegular(dataChangeEvent, partition)));
                            
            LOG.debug("Successfully partitioned event for table {} to partition {}", tableId, partition);
            
        } catch (Exception e) {
            LOG.error("Failed to partition event for table {}: {}", tableId, e.getMessage(), e);
            throw e;
        }
    }
    ...
    private Schema loadLatestSchemaFromRegistry(TableId tableId) {
        ...
        if (!schema.isPresent()) {
            LOG.warn("Schema not found for table {} in registry, attempting to wait and retry...", tableId);
            
            // Wait a bit and try again - schema might be in the process of being registered
            try {
                Thread.sleep(1000); // Wait 1 second
                schema = schemaEvolutionClient.getLatestEvolvedSchema(tableId);
                
                if (!schema.isPresent()) {
                    LOG.error("Schema still not available for table {} after retry", tableId);
                }
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
                LOG.warn("Interrupted while waiting for schema registration for table {}", tableId);
            } catch (Exception e) {
                LOG.warn("Error during schema retry for table {}: {}", tableId, e.getMessage());
            }
        }
