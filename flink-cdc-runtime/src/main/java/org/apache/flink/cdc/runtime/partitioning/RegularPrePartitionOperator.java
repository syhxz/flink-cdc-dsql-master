...
public class RegularPrePartitionOperator extends AbstractStreamOperator<PartitioningEvent>
    ...
    private void partitionBy(DataChangeEvent dataChangeEvent) throws Exception {
        ...
        try {
            ...
            output.collect(
                    new StreamRecord<>(PartitioningEvent.ofRegular(dataChangeEvent, partition)));

            LOG.debug(
                    "Successfully partitioned event for table {} to partition {}",
                    tableId,
                    partition);
    ...
    private Schema loadLatestSchemaFromRegistry(TableId tableId) {
        ...
        if (!schema.isPresent()) {
            LOG.warn(
                    "Schema not found for table {} in registry, attempting to wait and retry...",
                    tableId);
