...
public class DsqlRetryHelper {
    ...
    public <T> T executeWithRetry(Callable<T> operation, String operationName) throws Exception {
        ...
        for (int attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                if (attempt == maxRetries) {
                    LOG.error(
                            "Failed to execute {} after {} attempts", operationName, maxRetries, e);
                    break;
                }
        ...
        throw DsqlErrorReporter.reportRetryExhaustionError(
                operationName, maxRetries, totalDuration, lastException);
    }
