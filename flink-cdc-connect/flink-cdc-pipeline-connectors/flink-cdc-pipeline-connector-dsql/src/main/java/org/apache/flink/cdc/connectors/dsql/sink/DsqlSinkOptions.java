...
public class DsqlSinkOptions {
    ...
    public static final ConfigOption<String> SSL_MODE =
            ConfigOptions.key("ssl-mode")
                    .stringType()
                    .noDefaultValue()
                    .withDescription(
                            "SSL mode for the connection. Options: disable, allow, prefer, require, verify-ca, verify-full. "
                                    + "When use-iam-auth is true, defaults to 'require'. "
                                    + "When use-iam-auth is false, defaults to 'disable' (suitable for self-hosted PostgreSQL).");
    ...
    public static final ConfigOption<String> BATCH_TIMEOUT =
            ConfigOptions.key("batch-timeout")
                    ...
                    .defaultValue("5min")
                    .withDescription(
                            "Maximum time to wait before committing a partial batch (e.g., '5min', '30s').");
    ...
    public static final ConfigOption<Integer> BATCH_MAX_POOL_SIZE =
            ConfigOptions.key("batch-max-pool-size")
                    ...
                    .defaultValue(10)
                    .withDescription(
                            "Maximum number of connections in the batch processing connection pool.");
    ...
    public static final ConfigOption<Integer> BATCH_MIN_POOL_SIZE =
            ConfigOptions.key("batch-min-pool-size")
                    ...
                    .defaultValue(2)
                    .withDescription(
                            "Minimum number of connections in the batch processing connection pool.");
    ...
    public static final ConfigOption<Boolean> ENABLE_FULL_LOAD =
            ConfigOptions.key("enable-full-load")
                    ...
                    .defaultValue(true)
                    .withDescription(
                            "Whether to enable full load mode for initial data synchronization.");
    ...
    // Schema change (DDL synchronization) configuration
    public static final ConfigOption<String> SCHEMA_CHANGE_POLICY =
            ConfigOptions.key("schema-change.policy")
                    .stringType()
                    .defaultValue("evolve")
                    .withDescription(
                            "Policy for handling schema change events from source. "
                                    + "Options: 'evolve' (automatically apply DDL to target), "
                                    + "'ignore' (log and skip DDL changes), "
                                    + "'exception' (stop pipeline on schema change).");

    public static final ConfigOption<String> SCHEMA =
            ConfigOptions.key("schema")
                    .stringType()
                    .defaultValue("public")
                    .withDescription("Target schema name in the database.");
}
