...
/**
 * @deprecated This class is a non-functional prototype with hardcoded dummy values and incorrect
 *     SQL generation (e.g., "WHERE id = ?", "column0 = ?", "dummy_value_" parameters). Use {@link
 *     DsqlBatchSinkFunction} instead, which provides proper batching, connection pooling, correct
 *     SQL generation, and checkpoint integration.
 */
@Deprecated
public class DsqlSinkFunction implements SinkFunction<Event> {
            ...
            try (Connection connection = sinkProvider.createConnection()) {
                if (connection.isValid(5)) {
                    ...
                    if (event instanceof org.apache.flink.cdc.common.event.CreateTableEvent) {
                        processCreateTableEvent(
                                (org.apache.flink.cdc.common.event.CreateTableEvent) event,
                                connection);
                        ...
                        processDataChangeEvent(
                            ...
                            instanceof org.apache.flink.cdc.common.event.SchemaChangeEvent) {
    ...
    private void processCreateTableEvent(
            org.apache.flink.cdc.common.event.CreateTableEvent event, Connection connection)
            throws SQLException {
    ...
    private void processDataChangeEvent(
            org.apache.flink.cdc.common.event.DataChangeEvent event, Connection connection)
            throws SQLException {
    ...
    private void processSchemaChangeEvent(
            org.apache.flink.cdc.common.event.SchemaChangeEvent event, Connection connection)
            throws SQLException {
        ...
        LOG.warn(
                "Schema change events are not yet fully implemented: {}",
                event.getClass().getSimpleName());
    }
