...
public class PostgresSchemaDiscovery implements AutoCloseable {
    ...
    public List<TableId> discoverTables() throws Exception {
        ...
        try (Connection conn = getConnection()) {
                    ...
                    if (tableExists(metaData, tableId)) {
                        ...
                        throw new IllegalArgumentException(
                                "Table "
                                        ...
                                        + ". Please verify the table name and schema.");
                    }
                } catch (Exception e) {
                    LOG.error("Failed to discover table: {}", formattedTable, e);
                    throw new RuntimeException(
                            "Failed to discover table " + formattedTable + ": " + e.getMessage(),
                            e);
                }
            }
            ...
            throw new RuntimeException(
                    "Failed to connect to PostgreSQL database for table discovery: "
                            + e.getMessage(),
                    e);
        }
    ...
    public Schema getTableSchema(TableId tableId) throws Exception {
        try (Connection conn = getConnection()) {
            ...
            Schema.Builder schemaBuilder = Schema.newBuilder().setColumns(columns);
            ...
            LOG.info(
                    "Successfully discovered schema for table {}: {} columns, primary keys: {}",
                    tableId,
                    columns.size(),
                    primaryKeys);
            ...
            throw new RuntimeException(
                    "Unexpected error while getting schema for table "
                            + tableId
                            + ": "
                            + e.getMessage(),
                    e);
    ...
    private List<Column> getTableColumns(DatabaseMetaData metaData, TableId tableId)
            throws SQLException {
        ...
        try (ResultSet rs =
            ...
            while (rs.next()) {
                ...
                DataType flinkDataType =
                        mapPostgresTypeToFlinkType(dataType, typeName, columnSize, decimalDigits);

                Column column = Column.physicalColumn(columnName, flinkDataType, defaultValue);
                columns.add(column);

                LOG.debug(
                        "Discovered column: {} {} (nullable: {}, default: {})",
                        columnName,
                        flinkDataType,
                        nullable,
                        defaultValue);
    ...
    private List<String> getTablePrimaryKeys(DatabaseMetaData metaData, TableId tableId)
            throws SQLException {
    ...
    private DataType mapPostgresTypeToFlinkType(
        ...
        switch (sqlType) {
            ...
            default:
                LOG.warn(
                        "Unsupported PostgreSQL type: {} ({}), mapping to STRING",
                        typeName,
                        sqlType);
                return DataTypes.STRING();
        }
