...
public class PostgresPipelineSourceConfigFactory {
    private final org.apache.flink.cdc.connectors.postgres.source.config.PostgresSourceConfigFactory
            cdcFactory;
    ...
    public PostgresPipelineSourceConfigFactory() {
        this.cdcFactory =
                new org.apache.flink.cdc.connectors.postgres.source.config
    ...
    public PostgresPipelineSourceConfigFactory slotName(String slotName) {
        ...
        System.out.println(
                "DEBUG: PostgresPipelineSourceConfigFactory.slotName() called with: " + slotName);
    ...
    public PostgresPipelineSourceConfigFactory decodingPluginName(String decodingPluginName) {
        System.out.println(
                "DEBUG: PostgresPipelineSourceConfigFactory.decodingPluginName() called with: "
                        + decodingPluginName);
    ...
    private org.apache.flink.cdc.connectors.base.options.StartupOptions convertStartupOptions(
        ...
        switch (startupOptions.getMode()) {
            ...
            case SPECIFIC_OFFSET:
                ...
                // The CDC connector expects both file and position, but PostgreSQL LSN is a single
                // value
                // We'll use the LSN as the file parameter and 0 as position
                return org.apache.flink.cdc.connectors.base.options.StartupOptions.specificOffset(
                        startupOptions.getSpecificOffset(), 0);
            case TIMESTAMP:
                return org.apache.flink.cdc.connectors.base.options.StartupOptions.timestamp(
                        startupOptions.getTimestamp());
            default:
                throw new IllegalArgumentException(
                        "Unsupported startup mode: " + startupOptions.getMode());
        }
    }

    public org.apache.flink.cdc.connectors.postgres.source.config.PostgresSourceConfig create(
            int subtaskId) {
