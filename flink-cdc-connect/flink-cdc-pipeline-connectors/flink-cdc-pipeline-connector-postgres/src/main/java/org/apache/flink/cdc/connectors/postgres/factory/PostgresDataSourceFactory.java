...
import org.apache.flink.cdc.connectors.postgres.utils.PostgresDdlCaptureInitializer;
...
public class PostgresDataSourceFactory implements DataSourceFactory {
    ...
    public static final ConfigOption<String> DATABASE_NAME =
            key("database-name")
                    .stringType()
                    .noDefaultValue()
                    .withDescription("PostgreSQL database name");

    public static final ConfigOption<String> SCHEMA_NAME =
            key("schema-name")
                    .stringType()
                    .defaultValue("public")
                    .withDescription("PostgreSQL schema name");
    ...
    public static final ConfigOption<String> SLOT_NAME =
            key("slot.name")
                    ...
                    .withDescription("PostgreSQL replication slot name");
    ...
    public static final ConfigOption<String> SCAN_STARTUP_MODE =
            key("scan.startup.mode")
                    ...
                    .withDescription(
                            "Startup mode for PostgreSQL CDC source (snapshot, latest-offset, specific-offset, timestamp)");
    ...
    public static final ConfigOption<Long> SCAN_STARTUP_TIMESTAMP_MILLIS =
            key("scan.startup.timestamp-millis")
                    ...
                    .noDefaultValue()
                    .withDescription(
                            "Timestamp in milliseconds for startup when using timestamp mode");
    ...
    public static final ConfigOption<String> DECODING_PLUGIN_NAME =
            key("decoding.plugin.name")
                    ...
                    .withDescription(
                            "PostgreSQL logical decoding plugin name (pgoutput, wal2json, etc.)");

    public static final ConfigOption<Boolean> DDL_CAPTURE_ENABLED =
            key("ddl-capture.enabled")
                    .booleanType()
                    .defaultValue(false)
                    .withDescription(
                            "Whether to enable DDL capture via PostgreSQL event triggers. "
                                    + "When enabled, automatically creates event trigger and "
                                    + "dts_ddl_command table on source database to capture DDL changes. "
                                    + "Requires superuser or event trigger privileges. "
                                    + "Controlled by sink's schema-change.policy parameter.");
    @Override
    public DataSource createDataSource(Context context) {
        ...
        try {
            ...
            throw new IllegalArgumentException(
                    "PostgreSQL configuration validation failed: " + e.getMessage(), e);
        }
        ...
        boolean ddlCaptureEnabled = config.get(DDL_CAPTURE_ENABLED);

        LOG.info(
                "PostgreSQL configuration - hostname: {}, port: {}, database: {}, schema: {}, tables: {}, slot: {}, decoding plugin: {}, ddl-capture: {}",
                hostname,
                port,
                databaseName,
                schemaName,
        ...
        // Initialize DDL capture if enabled
        if (ddlCaptureEnabled) {
            LOG.info("DDL capture enabled - initializing event trigger on source database");
            try {
                PostgresDdlCaptureInitializer.initialize(
                        hostname, port, databaseName, username, password);
            } catch (Exception e) {
                LOG.error(
                        "Failed to initialize DDL capture. DDL changes will not be synchronized. "
                                + "Ensure the user has EVENT TRIGGER privileges. Error: {}",
                        e.getMessage());
                // Don't fail the pipeline — DDL capture is optional enhancement
            }
        }

        LOG.info(
                "PostgreSQL configuration - hostname: {}, port: {}, database: {}, schema: {}, tables: {}, slot: {}, decoding plugin: {}",
                hostname,
                port,
                databaseName,
                schemaName,
        ...
        LOG.info("  DDL_CAPTURE_ENABLED: {}", config.get(DDL_CAPTURE_ENABLED));
        ...
        try {
            ...
            // If DDL capture is enabled, add the DDL command table to monitoring list
            if (ddlCaptureEnabled) {
                String ddlTable = PostgresDdlCaptureInitializer.DDL_COMMAND_TABLE_FULL;
                boolean alreadyIncluded = false;
                for (String t : tableList) {
                    if (t.equals(ddlTable)) {
                        alreadyIncluded = true;
                        break;
                    }
                }
                if (!alreadyIncluded) {
                    String[] extendedList = Arrays.copyOf(tableList, tableList.length + 1);
                    extendedList[tableList.length] = ddlTable;
                    tableList = extendedList;
                    LOG.info("Added DDL command table to monitoring list: {}", ddlTable);
                }
            }

            LOG.info(
                    "Successfully created PostgreSQL source for database: {}, tables: {}",
                    databaseName,
                    Arrays.toString(tableList));

            // Create config factory for the pipeline connector
            PostgresPipelineSourceConfigFactory configFactory =
                    new PostgresPipelineSourceConfigFactory();
    ...
    public Set<ConfigOption<?>> optionalOptions() {
        ...
        options.add(DDL_CAPTURE_ENABLED);
        return options;
    }
