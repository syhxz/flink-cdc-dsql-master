...
public class PostgresStartupOptionsIntegrationTest {
    ...
    void testTimestampStartupMode() throws Exception {
        ...
        config.set(
                PostgresDataSourceFactory.SCAN_STARTUP_TIMESTAMP_MILLIS,
    ...
    void testSpecificOffsetStartupMode() throws Exception {
        ...
        config.set(
                PostgresDataSourceFactory.SCAN_STARTUP_SPECIFIC_OFFSET,
                "0/0"); // PostgreSQL LSN format
    ...
    void testStartupModeValidationBeforeConnection() {
        ...
        Configuration config = createBasicConfiguration();
        config.set(
                PostgresDataSourceFactory.HOSTNAME,
                "invalid-host"); // This would cause connection failure
        config.set(
                PostgresDataSourceFactory.SCAN_STARTUP_MODE,
                "invalid-mode"); // But this should fail first
        ...
        assertTrue(
                exception.getMessage().contains("startup mode")
