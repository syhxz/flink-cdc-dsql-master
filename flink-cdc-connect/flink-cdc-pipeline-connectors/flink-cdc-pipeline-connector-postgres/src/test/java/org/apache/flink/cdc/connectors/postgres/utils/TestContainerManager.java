...
/**
 * Robust container management system for PostgreSQL integration tests. Handles Docker availability
 * detection, container lifecycle management, and provides retry logic with exponential backoff.
 */
public class TestContainerManager {
    ...
    /**
     * Gets or creates a PostgreSQL container instance. If Docker is not available, skips the test
    ...
    public PostgreSQLContainer<?> getOrCreateContainer() {
        Assumptions.assumeTrue(
                dockerAvailable, "Docker is not available - skipping integration tests");
    ...
    private void startContainerWithRetry() {
        ...
        for (int attempt = 1; attempt <= MAX_STARTUP_ATTEMPTS; attempt++) {
            try {
                LOG.info(
                        "Starting PostgreSQL container (attempt {}/{})",
                        attempt,
                ...
                if (attempt < MAX_STARTUP_ATTEMPTS) {
                    try {
                        ...
                        throw new RuntimeException(
                                "Interrupted during container startup retry", ie);
                    }
                }
            }
        }
        ...
        throw new RuntimeException(
                "Failed to start PostgreSQL container after " + MAX_STARTUP_ATTEMPTS + " attempts",
                lastException);
    }

    /**
     * Performs additional health checks to ensure the container is ready for use. This goes beyond
    ...
    /**
     * Performs cleanup of container resources. This method is safe to call multiple times and
