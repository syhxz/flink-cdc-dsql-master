...
public class PostgresErrorScenariosIntegrationTest {
    ...
    void testConnectionFailures() {
        ...
        assertTrue(
                exception.getMessage().contains("Failed to connect")
    ...
    void testInvalidTableNameFormats() {
        ...
        assertTrue(
                exception.getMessage().contains("Invalid table")
    ...
    void testInvalidStartupModeConfigurations() {
        ...
        final DataSourceFactory.Context invalidTimestampContext =
                createContext(invalidTimestampConfig);
    ...
    void testDatabasePermissionIssues() throws Exception {
        ...
        try (Connection conn = DriverManager.getConnection(jdbcUrl, connectionProps);
            ...
            // Create a limited user (this might fail in some test environments, so we'll catch and
            // skip)
            try {
                stmt.execute("CREATE USER limited_user WITH PASSWORD 'limited_pass'");
                stmt.execute(
                        "GRANT CONNECT ON DATABASE "
                                + containerManager.getDatabaseName()
                                + " TO limited_user");
