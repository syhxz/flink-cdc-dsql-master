...
public class PostgresStreamingChangeEventSource
    ...
    /** Maximum number of reconnection attempts during streaming before giving up. */
    ...
    public void execute(
        ...
        while (context.isRunning()) {
            try {
                ...
                // Reset reconnect counter on successful streaming start
                reconnectAttempts = 0;

                processMessages(context, partition, offsetContext, stream);

                // If processMessages returned normally (e.g., bounded read finished), clean up and
                // exit
                cleanupStreamingResources(offsetContext);
                break;

            } catch (Throwable e) {
                // Determine if this is a connection error that can be recovered via reconnect
                if (isRetryableConnectionError(e) && context.isRunning()) {
                    reconnectAttempts++;
                    if (reconnectAttempts > MAX_RECONNECT_ATTEMPTS) {
                        LOGGER.error(
                                "Exceeded maximum reconnection attempts ({}) after connection loss. Failing the task.",
                    ...
                    long delayMs =
                            Math.min(
                                    RECONNECT_INITIAL_DELAY_MS * (1L << (reconnectAttempts - 1)),
                                    RECONNECT_MAX_DELAY_MS);
                    LOGGER.warn(
                            "Connection lost during streaming (attempt {}/{}). "
                                    + "Reconnecting in {}ms from LSN '{}'. Error: {}",
                            reconnectAttempts,
                            MAX_RECONNECT_ATTEMPTS,
                            delayMs,
                    ...
                    try {
                        ...
                        LOGGER.info("Interrupted during reconnect delay");
                        errorHandler.setProducerThrowable(ie);
                        break; // cleanupStreamingResources already called above
                    }

                    // Reconnect the replication connection
                    try {
                        replicationConnection.reconnect();
                        LOGGER.info(
                                "Successfully reconnected replication connection (attempt {}/{})",
                                reconnectAttempts,
                                MAX_RECONNECT_ATTEMPTS);
                        // Update context so next iteration uses the last processed LSN
                        hasStartLsnStoredInContext = true;
                    } catch (Exception reconnectException) {
                        LOGGER.error(
                                "Failed to reconnect replication connection: {}",
                                reconnectException.getMessage());
                        errorHandler.setProducerThrowable(e);
                        break; // cleanupStreamingResources already called above
                    }
                } else {
                    // Non-retryable error or context stopped — propagate immediately
                    errorHandler.setProducerThrowable(e);
                    cleanupStreamingResources(offsetContext);
                    break;
                }
        ...
        // Final cleanup: close the replication connection when exiting the streaming loop
        if (replicationConnection != null) {
            try {
                replicationConnection.close();
            } catch (Exception e) {
                LOGGER.debug("Exception while closing replication connection on exit", e);
            }
        }
    }
    ...
    /**
     * Determines whether an exception represents a retryable connection error (e.g., failover,
     * network interruption) vs a non-retryable logical error.
     */
    private boolean isRetryableConnectionError(Throwable e) {
        // Walk the exception chain to find connection-related errors
        Throwable current = e;
        while (current != null) {
            // Socket/IO errors indicate connection loss
            ...
            if (current instanceof SQLException) {
                ...
                if (sqlState != null) {
                    ...
                    // 57P01 admin_shutdown (server shutting down for failover)
            ...
            // Check for known connection-loss messages
            ...
            if (message != null) {
                ...
                if (lowerMsg.contains("connection reset")
                        ...
                        || lowerMsg.contains("unexpected eof")
    ...
    /**
     * Cleans up streaming resources (keep-alive thread, replication stream) safely. This is called
     * both during normal shutdown and before reconnection attempts.
     */
    private void cleanupStreamingResources(PostgresOffsetContext offsetContext) {
        if (replicationConnection != null) {
            ...
            try {
                ...
                LOGGER.debug("Exception while committing during cleanup", e);
            }
