...
public class PostgresScanFetchTask extends AbstractScanFetchTask {
    ...
    public void execute(Context context) throws Exception {
        ...
        // Log overall snapshot task start
        LOG.info("=== POSTGRES SNAPSHOT TASK STARTING ===");
        LOG.info("Slot Name: {}", sourceConfig.getSlotNameForBackfillTask());
        LOG.info("Table: {}", snapshotSplit.getTableId());
        LOG.info("Split ID: {}", snapshotSplit.splitId());
        ...
        try {
            ...
            LOG.info("=== POSTGRES SNAPSHOT TASK COMPLETED ===");
            LOG.info("Slot Name: {}", sourceConfig.getSlotNameForBackfillTask());
            LOG.info("Table: {}", snapshotSplit.getTableId());
        } finally {
    ...
    public static class PostgresSnapshotSplitReadTask
        ...
        // Static field to store snapshot begin LSN for CDC streaming start
        // This is a temporary solution - in production, this should be communicated through proper channels
        private static volatile PostgresOffset globalSnapshotBeginOffset = null;
        ...
        protected SnapshotResult<PostgresOffsetContext> doExecute(
            ...
            // Capture the snapshot begin LSN (xlogStart) for CDC streaming start
            if (globalSnapshotBeginOffset == null) {
                try {
                    // Get the current xlogStart LSN when snapshot begins
                    Long lsn = jdbcConnection.currentXLogLocation();
                    Long txId = jdbcConnection.currentTransactionId();
                    // Use the public static factory method to create PostgresOffset
                    Map<String, Object> offsetMap = new HashMap<>();
                    offsetMap.put("lsn", lsn.toString());
                    offsetMap.put("txId", txId.toString());
                    globalSnapshotBeginOffset = PostgresOffset.of(offsetMap);
                    
                    LOG.info("=== SNAPSHOT BEGIN LSN CAPTURED ===");
                    LOG.info("Snapshot Begin LSN: LSN{{{}/{:X}}}", lsn >>> 32, lsn & 0xFFFFFFFFL);
                    LOG.info("Transaction ID: {}", txId);
                    LOG.info("This LSN will be used for CDC streaming start");
                    
                } catch (SQLException e) {
                    LOG.warn("Failed to capture snapshot begin LSN: {}", e.getMessage());
                }
            }

            // Log snapshot begin with slot name and LSN position
            String slotName = connectorConfig.getLogicalName(); // Use logical name instead
            String currentLsn = ctx.offset != null ? ctx.offset.toString() : "UNKNOWN";
            LOG.info("=== SNAPSHOT BEGIN ===");
            LOG.info("Slot Name: {}", slotName);
            LOG.info("Starting LSN Position: {}", currentLsn);
            LOG.info("Table: {}", snapshotSplit.getTableId());
            LOG.info("Split Range: {} to {}", snapshotSplit.getSplitStart(), snapshotSplit.getSplitEnd());

            refreshSchema(databaseSchema, jdbcConnection, true);
            createDataEvents(ctx, snapshotSplit.getTableId());

            // Log snapshot end with final LSN position
            String finalLsn = ctx.offset != null ? ctx.offset.toString() : "UNKNOWN";
            LOG.info("=== SNAPSHOT END ===");
            LOG.info("Slot Name: {}", slotName);
            LOG.info("Final LSN Position: {}", finalLsn);
            LOG.info("Table: {}", snapshotSplit.getTableId());
            
            if (globalSnapshotBeginOffset != null) {
                LOG.info("CDC will start from snapshot begin LSN: {} instead of high watermark", 
                    globalSnapshotBeginOffset.getLsn());
            } else {
                LOG.info("CDC will start from LSN: {}", finalLsn);
            }
        ...
        /**
         * Get the globally stored snapshot begin offset.
         * This is used by the HybridSplitAssigner to start CDC streaming from snapshot begin LSN.
         */
        public static PostgresOffset getGlobalSnapshotBeginOffset() {
            return globalSnapshotBeginOffset;
        }
        
        /**
         * Reset the global snapshot begin offset.
         * This should be called when starting a new job to ensure clean state.
         */
        public static void resetGlobalSnapshotBeginOffset() {
            globalSnapshotBeginOffset = null;
            LOG.info("Reset global snapshot begin offset");
        }
