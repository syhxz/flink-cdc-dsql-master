...
public class PostgresStreamFetchTask implements FetchTask<SourceSplitBase> {
    ...
    public void execute(Context context) throws Exception {
        ...
        // Log CDC streaming start with slot name and LSN position
        String slotName = sourceFetchContext.getDbzConnectorConfig().getLogicalName();
        String startingLsn = split.getStartingOffset() != null ? 
            split.getStartingOffset().toString() : "UNKNOWN";
        LOG.info("=== CDC STREAMING STARTING ===");
        LOG.info("Slot Name: {}", slotName);
        LOG.info("Starting LSN Position: {}", startingLsn);
        LOG.info("Stream Split ID: {}", split.splitId());
