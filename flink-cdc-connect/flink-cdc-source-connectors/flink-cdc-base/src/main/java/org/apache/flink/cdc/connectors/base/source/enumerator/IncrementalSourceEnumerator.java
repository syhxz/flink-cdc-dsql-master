...
public class IncrementalSourceEnumerator
    ...
    public void start() {
        ...
        if (splitAssigner instanceof HybridSplitAssigner) {
            try {
                ...
                LOG.info(
                        "Test: HybridSplitAssigner is ready to receive snapshot begin offset for CDC streaming start");
            } catch (Exception e) {
    ...
    public void handleSourceEvent(int subtaskId, SourceEvent sourceEvent) {
        if (sourceEvent instanceof FinishedSnapshotSplitsReportEvent) {
            ...
            LOG.info(
                    ...
                    subtaskId,
                    ((SnapshotBeginEvent) sourceEvent).getSnapshotBeginOffset());
            SnapshotBeginEvent beginEvent = (SnapshotBeginEvent) sourceEvent;
            if (splitAssigner instanceof HybridSplitAssigner) {
                ((HybridSplitAssigner<?>) splitAssigner)
                        .setSnapshotBeginOffset(beginEvent.getSnapshotBeginOffset());
            }
        }
    }
    ...
    /**
     * Triggers an automatic checkpoint when snapshot splits are finished to prevent deadlock. This
     * method uses the SourceEnumeratorContext to request a checkpoint programmatically.
     */
    private void triggerAutoCheckpointIfNeeded() {
        ...
        if (splitAssigner.noMoreSplits() && !autoCheckpointTriggered) {
            ...
            LOG.info(
                    "🔧 AUTO-CHECKPOINT: All snapshot splits finished. Starting auto-checkpoint mechanism...");

            // Start a background thread to handle auto-checkpoint with delay
            Thread autoCheckpointThread =
                    new Thread(
                            () -> {
                                try {
                                    ...
                                    if (splitAssigner.noMoreSplits() && !isStreamSplitAssigned()) {
                                        LOG.warn(
                                                "🚨 AUTO-CHECKPOINT: No checkpoint triggered after 2 minutes. Requesting checkpoint...");

                                        // Request checkpoint through SourceEnumeratorContext
                                        try {
                                            // Use reflection to access checkpoint triggering if
                                            // available
                                            triggerCheckpointThroughContext();
                                        } catch (Exception e) {
                                            LOG.warn(
                                                    "Failed to trigger checkpoint through context: {}",
                                                    e.getMessage());
                                        ...
                                        LOG.info(
                                                "✅ AUTO-CHECKPOINT: Normal checkpoint flow detected, auto-trigger not needed");
                                    }
    ...
    private void triggerCheckpointThroughContext() {
        LOG.info(
                "🔧 AUTO-CHECKPOINT: Attempting to trigger checkpoint through SourceEnumeratorContext");
        ...
        context.callAsync(
                ...
                (result, throwable) -> {
                    if (throwable != null) {
                        LOG.warn(
                                "🚨 AUTO-CHECKPOINT: Async checkpoint request failed: {}",
                                throwable.getMessage());
                    } else {
                        LOG.info(
                                "✅ AUTO-CHECKPOINT: Async checkpoint request completed: {}",
                                result);
                    }
                });
    }
            ...
            context.callAsync(
                    ...
                    (result, throwable) -> {
                        if (!isStreamSplitAssigned() && splitAssigner.noMoreSplits()) {
                            LOG.info(
                                    "🔄 AUTO-CHECKPOINT: Periodic request #{} - still waiting for checkpoint",
                                    attempt);

                            if (attempt >= 12) { // After 2 minutes total
                                LOG.warn(
                                        "🚨 AUTO-CHECKPOINT: Checkpoint still not triggered after 2 minutes");
                                LOG.warn(
                                        "🚨 This indicates a potential checkpoint configuration issue");
                            }
                        } else {
                            LOG.info(
                                    "✅ AUTO-CHECKPOINT: Checkpoint completed, stopping periodic requests");
                        }
                    });
        }
    }
