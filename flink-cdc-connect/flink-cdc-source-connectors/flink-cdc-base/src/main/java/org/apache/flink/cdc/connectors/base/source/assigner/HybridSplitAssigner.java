...
public class HybridSplitAssigner<C extends SourceConfig> implements SplitAssigner {
    ...
    public void open() {
        ...
        if (snapshotBeginOffset == null) {
            // For now, use a placeholder offset - this will be replaced with actual snapshot begin
            // LSN
    ...
    /**
     * Set the snapshot begin offset for CDC streaming start. This should be called when snapshot
     * begins to capture the xlogStart LSN.
     */
    ...
    public StreamSplit createStreamSplit() {
        ...
        if (snapshotBeginOffset != null) {
            ...
            LOG.info(
                    "CDC streaming will start from stored snapshot begin LSN: {}",
                    snapshotBeginOffset);
        } else {
            ...
            try {
                ...
                Class<?> fetchTaskClass =
                        Class.forName(
                                "org.apache.flink.cdc.connectors.postgres.source.fetch.PostgresScanFetchTask$PostgresSnapshotSplitReadTask");
                java.lang.reflect.Method getOffsetMethod =
                        fetchTaskClass.getMethod("getGlobalSnapshotBeginOffset");
                Object postgresOffset = getOffsetMethod.invoke(null);

                if (postgresOffset != null) {
                    // Convert PostgresOffset to generic Offset
                    // This is a simplified conversion - in production, proper conversion should be
                    // used
                    streamStartOffset = offsetFactory.createInitialOffset(); // Placeholder
                    LOG.info(
                            "CDC streaming will start from PostgreSQL snapshot begin LSN: {}",
                            postgresOffset);
                } else {
                    streamStartOffset =
                            minOffset == null ? offsetFactory.createInitialOffset() : minOffset;
                    LOG.warn(
                            "PostgreSQL snapshot begin offset not available, using minimum finished offset: {}",
                            streamStartOffset);
                }
            } catch (Exception e) {
                ...
                streamStartOffset =
                        minOffset == null ? offsetFactory.createInitialOffset() : minOffset;
                LOG.warn(
                        "Failed to access PostgreSQL snapshot begin offset, using minimum finished offset: {} (error: {})",
                        streamStartOffset,
                        e.getMessage());
            }
        }
