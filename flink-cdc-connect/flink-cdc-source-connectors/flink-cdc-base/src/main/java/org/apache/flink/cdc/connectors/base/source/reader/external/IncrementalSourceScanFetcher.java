...
public class IncrementalSourceScanFetcher implements Fetcher<SourceRecords, SourceSplitBase> {
    ...
    public Iterator<SourceRecords> pollWithoutBuffer() throws InterruptedException {
        ...
        for (DataChangeEvent event : batch) {
            SourceRecord record = getSourceRecordFromEvent(event);
            if (record != null && isEndWatermarkEvent(record)) {
    ...
    private SourceRecord getSourceRecordFromEvent(DataChangeEvent event) {
        try {
            // Try to get the record using reflection to handle both shaded and unshaded versions
            Object record = event.getClass().getMethod("getRecord").invoke(event);
            if (record instanceof SourceRecord) {
                return (SourceRecord) record;
            } else {
                // If it's a shaded SourceRecord, we need to handle it differently
                // For now, let's try to cast it or return null to skip
                LOG.warn("Received non-standard SourceRecord type: {}", record.getClass().getName());
                return null;
            }
        } catch (Exception e) {
            LOG.error("Failed to get SourceRecord from DataChangeEvent", e);
            return null;
        }
    }
}
